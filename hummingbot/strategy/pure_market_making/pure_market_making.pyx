...
cdef class PureMarketMakingStrategy(StrategyBase):
    ...
    def init_params(self,
                    ...
                    micro_price_price_source: bool = False,
                    ...
                    target_base_balance: decimal = Decimal(0),
                    ...
                    hanging_orders_enabled_other : bool = True,
                    min_profitability: Decimal = Decimal(0.5),
                    hanging_inventory_management_range: Decimal = Decimal(0.5),
                    moving_price_band: Optional[MovingPriceBand] = None,
                    volatility_buffer_size: int = 200,

                    volatility_processing_length: int = 200,
                    volatility_adjustment: bool = False,
                    max_volatility_spread: Decimal = Decimal(0.7),
                    max_inventory_management_spread: Decimal = Decimal(0.7),
                    volatility_adjustment_multiplier: Decimal = Decimal(1),
        ...
        self._hanging_inventory_management_range = hanging_inventory_management_range
        ...
        self._max_volatility_spread = max_volatility_spread
        self._max_inventory_management_spread = max_inventory_management_spread
        self._volatility_adjustment_multiplier = volatility_adjustment_multiplier
        self._inventory_management = inventory_management
        ...
        self._avg_vol = AverageVolatilityIndicator(sampling_length=volatility_buffer_size, processing_length=volatility_processing_length)
    ...
    def get_price(self) -> Decimal:
        ...
        if self._price_type is PriceType.LastOwnTrade:
          ...
          if self._conversion_data_source:
            price = price = price_provider.get_price_by_type(PriceType.MidPrice) / self._conversion_market.get_mid_price()
          else:
            price = price = price_provider.get_price_by_type(PriceType.MidPrice)

        else:
          if self._conversion_data_source:
            price = price_provider.get_price_by_type(self._price_type) / self._conversion_market.c_get_mid_price()
          else:
            price = price_provider.get_price_by_type(self._price_type)

        if price.is_nan():
          if self._conversion_data_source:
            price = price_provider.get_price_by_type(PriceType.MidPrice) / self._conversion_market.c_get_mid_price()
          else:
    ...
    cdef object c_get_mid_price(self):
        ...
        if self._asset_price_delegate is not None:
          if self._conversion_data_source:
            mid_price = delegate.c_get_mid_price() / self._conversion_market.c_get_mid_price()
          else:
            mid_price = delegate.c_get_mid_price()
        else:
          if self._conversion_data_source:
            mid_price = self._market_info.get_mid_price() / self._conversion_market.c_get_mid_price()
          else:
    ...
    def market_status_data_frame(self, market_trading_pair_tuples: List[MarketTradingPairTuple]) -> pd.DataFrame:
        markets_data = []
        markets_columns = ["Exchange", "Market", "Best Bid", "Best Ask", f"Ref Price ({self._price_type.name})", "micro-price","volatility_percentage"]
        ...
        for market, trading_pair in market_books:
            ...
            markets_data.append([
                ...
                float(ask_price),
                float(ref_price),
                round(self.c_weighted_mid_price(),5),
                round((self.get_volatility() / ref_price),5)
    ...
    cdef c_tick(self, double timestamp):
        """
        Main process. Logic depends on the keep_target_balance_setting and the hanging_order_other setting.
        Keep_target_balance: define target balance, when order is filled, place hanging order other at the bid/ask spread away from the order fill. If not filled within the filled_order_delay_target_balance time. Cancel order and place orders on top bid and ask level to restore balance_warning
        Therefore there is another logic in there that does not cancel the orders whenever a limit is reached. This logic is only active when the keep_target_balance is active_buys

        Other logic is the normal logic.
        """
    ...
    cdef object c_create_base_proposal(self):
          ...
          if self._use_micro_price:
            ...
            if adjusted_micro_price > 0:
              buy_reference_price = sell_reference_price = adjusted_micro_price
          ...
          if order_override is not None and len(order_override) > 0:
            ...
            if self.c_check_imbalance() or not self._keep_target_balance:
            ...
            if not self.c_check_imbalance() and (not self._buy_order_fill and not self._sell_order_fill) and self._keep_target_balance:
              base_balance = float(market.get_balance(self._market_info.base_asset))
              deviation = base_balance - float(self._target_base_balance)
              mid_price = self._market_info.get_mid_price()

              if deviation < 0:
                  for level in range(0, 1): #only 1 order
                      price = (mid_price / (Decimal(1) + (self._bid_spread * (Decimal(1) - self._target_balance_spread_reducer))) - (level * self._order_level_spread))
                      price = market.c_quantize_order_price(self.trading_pair, price)
    ...
    cdef c_apply_inventory_spread_management(self, object proposal):
      ...
      """
      Adjust the price of the bid and asks based on the inventory skew
      It records the current spread per bid and ask order
      It records the current inventory skew. This value (at a 50% base asset target) ranges between -0.5 and 0.5. If value is above 0, you have more base asset than quote, so you should be selling rather than buying
      This value multiplied by the inventory acts as an increaser/decreaser of the current spreads. E.g. spread on both sides 0.5% mulitplier of 1 and inventory_value of 0.25.
      IMPORTANT: In contrart to the c_apply_vol_adjustment function, this is a multiplier of the current spread rather than increasing the price with an absolute value
      """

      cdef:
        ExchangeBase market = self._market_info.market
        mid_price = self.get_price()
        inventory_value = self.get_inventory_management()

      if inventory_value <= 0:
        spread_adjustment_bid = Decimal(1) - abs((inventory_value * self._inventory_management_multiplier))
        spread_adjustment_ask = (Decimal(1) + abs((inventory_value * self._inventory_management_multiplier)))

      if inventory_value > 0:
          spread_adjustment_bid = (Decimal(1) + (inventory_value * self._inventory_management_multiplier))
          spread_adjustment_ask = Decimal(1) - (inventory_value * self._inventory_management_multiplier)

      if len(proposal.buys) != 0:
        if (self.get_first_order_proposal_spread(proposal)[0] * spread_adjustment_bid) < self._max_inventory_management_spread:
          if spread_adjustment_bid < 0:
            spread_adjustment_bid = Decimal(0.01)
            self.log_with_clock(
                  logging.INFO,
                  f"Inventory_spread_Adjustment: bid spread exceeds limit"
              )

        else:
          spread_adjustment_bid = (Decimal(1) - ((self._max_inventory_management_spread - self.get_first_order_proposal_spread(proposal)[0]) / self.get_first_order_proposal_spread(proposal)[0]))

      if len(proposal.sells) != 0:
        if (self.get_first_order_proposal_spread(proposal)[1] * spread_adjustment_ask) < self._max_inventory_management_spread:
          if spread_adjustment_ask < 0:
            spread_adjustment_ask = Decimal(0.01)
            self.log_with_clock(
                  logging.INFO,
                  f"Inventory_spread_Adjustment: ask spread exceeds limit"
              )
        else:
          spread_adjustment_ask = (Decimal(1) + ((self._max_inventory_management_spread - self.get_first_order_proposal_spread(proposal)[1]) / self.get_first_order_proposal_spread(proposal)[1]))

      self.log_with_clock(
            logging.INFO,
            f"Inventory_spread_Adjustment: spread_adjustment_bid {spread_adjustment_bid} - spread_adjustment_ask {spread_adjustment_ask}"
        )
      ...
      for sell in proposal.sells:
          price = sell.price
          current_spread = (price - mid_price) / mid_price
          new_spread = current_spread * spread_adjustment_ask
          inventory_adjusted_price = mid_price * (Decimal(1) + new_spread)
          sell.price = market.c_quantize_order_price(self.trading_pair, inventory_adjusted_price)

    def get_first_order_proposal_spread(self, object proposal):
      """
      Return the first bid and ask spread (e.g. 0.004 for 0.4% spread) based on closeness to the mid_price.
      Usef in volatility adjustment and inventory management adjustments
      """

      mid_price = self.get_price()

      proposal.buys = sorted(proposal.buys, key = lambda p: p.price, reverse = True)
      proposal.sells = sorted(proposal.sells, key = lambda p: p.price, reverse = False)

      first_bid_order_spread = Decimal((mid_price - proposal.buys[0].price) / mid_price)
      first_ask_order_spread = Decimal((proposal.sells[0].price - mid_price) / mid_price)
    ...
    cdef c_did_fill_order(self, object order_filled_event):
        ...
        if market_info is not None:
            ...
            if order_filled_event.trade_type is TradeType.BUY:
              ''' hanging order section - should be recreated as a function'''
              if not self._hanging_orders_enabled and self._hanging_orders_enabled_other:
                if self._buy_order_fill and self.current_timestamp >= self._balance_fixer_timestamp:
                  ...
                  if (order_id not in self._hanging_order_list): #positive = you need to sell
                    if (self._inventory_management and (self.get_inventory_management() > (Decimal(self._hanging_inventory_management_range)*Decimal(-1)))) or not self._inventory_management:
                      top_ask = market.c_get_price(self.trading_pair, True)
                      ask_order_id = self.c_sell_with_specific_market(
                          self._market_info,
                          amount = market.c_quantize_order_amount(self.trading_pair, order_filled_event.amount),
                          order_type=self._limit_order_type,
                          price= market.c_quantize_order_price(self.trading_pair, max((order_filled_event.price * (Decimal(1) + self._min_profitability)),top_ask)),
                          expiration_seconds=expiration_seconds)
                      self._hanging_order_list.append(ask_order_id)
                      self.log_with_clock(
                          logging.INFO,
                          f"Created Sell Hanging Order({ask_order_id}) of {order_filled_event.amount}")
                    else:
                      self.log_with_clock(
                          logging.INFO,
                          f"Did not create hanging sell order as inventory management is already too skewed. Max: {Decimal(self._hanging_inventory_management_range)*Decimal(-1)}, Current: {self.get_inventory_management()}")

                    self.c_cancel_all_orders()

                self._last_buy_order_price = order_filled_event.price
                self.log_with_clock(
                    logging.INFO,
            ...
            if self._logging_options & self.OPTION_LOG_MAKER_ORDER_FILLED:
              ...
              if not self._hanging_orders_enabled and self._hanging_orders_enabled_other:
                  if self._sell_order_fill and self.current_timestamp >= self._balance_fixer_timestamp: #if it is already true, set it to false. second argument for if two orders are filled at the same time, if this happens, the timer is just reset
                    ...
                    if (order_id not in self._hanging_order_list):
                    #if inventory is too much skewed, you do not want to do hanging orders. If inventory_management and skew smaller than x then do not place the order. Inventory_management < 1 = too few base asset -> you need to sell
                      if (self._inventory_management and (self.get_inventory_management() < Decimal(self._hanging_inventory_management_range))) or not self._inventory_management:
                        top_bid = market.c_get_price(self.trading_pair, False)
                        bid_order_id = self.c_buy_with_specific_market(
                            self._market_info,
                            amount = market.c_quantize_order_amount(self.trading_pair, order_filled_event.amount),
                            order_type=self._limit_order_type,
                            price= market.c_quantize_order_price(self.trading_pair, min((order_filled_event.price / (Decimal(1) + self._min_profitability)), top_bid)),
                            expiration_seconds=expiration_seconds)
                        self._hanging_order_list.append(bid_order_id)
                        self.log_with_clock(
                          logging.INFO,
                          f"Created Buy Hanging Order({bid_order_id}) of {order_filled_event.amount}")
                      else:
                        self.log_with_clock(
                            logging.INFO,
                            f"Did not create hanging buy order as inventory management is already too skewed. Max: {Decimal(self._hanging_inventory_management_range)}, Current: {self.get_inventory_management()}")
                      self.c_cancel_all_orders()


                  self._last_sell_order_price = order_filled_event.price
                  self.log_with_clock(
                      logging.INFO,
                      f"self._buy_order_fill: order_id {order_id} - {self._sell_order_fill}) self._buy_order_fill: {self._buy_order_fill} self._last_buy_order_price : {self._last_buy_order_price} inventory_management {self.get_inventory_management()} "
                  )
        ...
        timestamp = order_filled_event.timestamp
        trading_pair = market_info.trading_pair
        exchange = self._exchange
        strategy = self._strategy
        strategy_identifier = self._strategy_identifier
        base_asset, quote_asset = trading_pair.split('-')
        buy =  (order_filled_event.trade_type == TradeType.BUY)
        type = order_filled_event.order_type
        amount = order_filled_event.amount
        price = order_filled_event.price
        fee_percent = order_filled_event.trade_fee.percent
        fee_percent_token = order_filled_event.trade_fee.percent_token
        flat_fee_token = order_filled_event.trade_fee.flat_fees[0]['token'] if order_filled_event.trade_fee.flat_fees != [] else None
        flat_fee_amount = order_filled_event.trade_fee.flat_fees[0]['amount'] if order_filled_event.trade_fee.flat_fees != [] else None
        mid_price = mid_price

        trade_fill_df = pd.DataFrame({
              "timestamp":[timestamp],
              "order_id":[order_id],
              ...
              "fee_percent_token": [fee_percent_token],
              "flat_fee_amount": [flat_fee_amount],
              "flat_fee_token": [flat_fee_token],
              "mid_price": [mid_price]
              })

        trade_fill_df.to_sql('TRADES_DF', con=self._connection, if_exists='append', index=False)

        self.log_with_clock(
             logging.INFO,
             f"Trade Fill Event succesfully stored in Database"
         )
    ...
    cdef c_cancel_active_orders(self, object proposal):
        ...
        if self._hanging_orders_enabled_other and self._current_timestamp >= self._balance_fixer_timestamp: #cancel haning orders
          for order in self.active_non_hanging_orders:
            if order.client_order_id in self._hanging_order_list:
              ...
              self.log_with_clock(
                  logging.INFO,
                  f"Cancelled Hanging Order {order.client_order_id}")
        ...
        if not to_defer_canceling:
            ...
            for order in self.active_non_hanging_orders:
                ...
                if not self._hanging_orders_tracker.is_potential_hanging_order(order) and (order.client_order_id not in self._hanging_order_list):
                  self.c_cancel_order(self._market_info, order.client_order_id)
                else:
                  pass
    ...
    cdef c_cancel_orders_below_min_spread(self):
        ...
        for order in active_orders:
            ...
            if order.client_order_id not in self._min_spread_order_id:
    ...
    cdef c_cancel_all_orders(self): #only the non haning ones
          ...
          for order in active_orders:
            if order.client_order_id not in self._hanging_order_list:
              ...
              self.log_with_clock(
                  logging.INFO,
                  f"Hanging Order {order.client_order_id} was not cancelled during cancel all")

    cdef bint c_to_create_orders(self, object proposal):
        amount_hanging_orders_other = 0
        non_hanging_orders_non_cancelled = [o for o in self.active_non_hanging_orders if not
                                            self._hanging_orders_tracker.is_potential_hanging_order(o)]
        for order in self.active_non_hanging_orders:
          if order.client_order_id in  self._hanging_order_list:
            amount_hanging_orders_other += 1
        return ((not self._should_wait_order_cancel_confirmation or
                     len(self._sb_order_tracker.in_flight_cancels) == 0)
                and proposal is not None
                and (len(non_hanging_orders_non_cancelled) - amount_hanging_orders_other) == 0)
    ...
    cdef c_execute_orders_proposal(self, object proposal):
        ...
        if len(proposal.buys) > 0:
            ...
            for idx, buy in enumerate(proposal.buys):
                ...
                orders_created = True
                if abs((self.get_mid_price() - buy.price) / self.get_mid_price()) <= self._minimum_spread:
                  self._min_spread_order_id.append(bid_order_id)
        ...
        if len(proposal.sells) > 0:
            ...
            for idx, sell in enumerate(proposal.sells):
                ...
                orders_created = True
                if abs((self.get_mid_price() - buy.price) / self.get_mid_price()) <= self._minimum_spread:
                  self._min_spread_order_id.append(ask_order_id)
                if idx < number_of_pairs:
