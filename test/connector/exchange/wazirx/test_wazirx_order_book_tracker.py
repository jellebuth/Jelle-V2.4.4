...
class WazirxOrderBookTrackerUnitTest(unittest.TestCase):
    order_book_tracker: Optional[WazirxOrderBookTracker] = None
    ...
    def setUpClass(cls):
        ...
        cls.order_book_tracker: WazirxOrderBookTracker = WazirxOrderBookTracker(cls.trading_pairs)
    ...
    def test_tracker_integrity(self):
        ...
        eth_usdt: OrderBook = order_books["ETH-WRX"]
    ...
    def test_api_get_last_traded_prices(self):
        prices = self.ev_loop.run_until_complete(
            WazirxAPIOrderBookDataSource.get_last_traded_prices(["BTC-INR", "ETH-WRX"]))
        for key, value in prices.items():
            print(f"{key} last_trade_price: {value}")
        self.assertGreater(prices["BTC-INR"], 1000)
        self.assertLess(prices["ETH-WRX"], 1)
